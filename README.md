...
|---|---|---|---|
| 1 | Команда, яку ввела людина | `curl -v https://example.net`<br><br>`curl -v http://neverssl.com`<br><br>`Resolve-DnsName example.net`<br><br>`curl -v https://google.com`<br><br>`curl -v https://expired.badssl.com`<br><br>`curl -v https://wrong.host.badssl.com`<br><br>`curl -v https://self-signed.badssl.com` | Це єдині рядки, які написала людина. Усе інше у виводах є реакцією комп'ютерів на ці команди, тому група найближча до користувача. |
| 2 | Діалог із веб-сервером: що просимо й що отримуємо | **Запит:**<br>`using HTTP/1.x`<br>`> GET / HTTP/1.1`<br>`> Host: ...`<br>`> User-Agent: curl/8.21.0`<br>`> Accept: */*`<br><br>**Відповідь:**<br>`< HTTP/1.1 200 OK`<br>`< HTTP/1.1 301 Moved Permanently`<br>`< Location: https://www.google.com/`<br>`< Date`, `< Content-Type`, `< Server`<br>`< Cache-Control`, `< cf-cache-status: HIT`<br>`< Alt-Svc` та інші рядки з `<`<br><br>**Вміст сторінки:**<br>`<!doctype html>...` (example.net)<br>`<html>...NeverSSL...` (neverssl.com)<br>`<HTML>...301 Moved...` (google.com) | Клієнт просить сторінку (`>`), сервер називає результат (200 чи 301), описує відповідь у заголовках і віддає вміст (`<`). Результат цього діалогу людина бачить у браузері, тому група розташована одразу після команди. |
| 3 | Домовленість про безпечний зв'язок і перевірка сертифіката | **Узгодження захисту:**<br>`schannel: disabled automatic use of client certificate`<br>`ALPN: curl offers http/1.1`<br>`ALPN: server accepted http/1.1`<br>`schannel: remote party requests renegotiation`<br>`schannel: renegotiating SSL/TLS connection`<br>`schannel: SSL/TLS connection renegotiated`<br><br>**Збої перевірки сертифіката:**<br>`SEC_E_CERT_EXPIRED`<br>`SEC_E_WRONG_PRINCIPAL`<br>`SEC_E_UNTRUSTED_ROOT` | Сторони узгоджують шифрування, а клієнт перевіряє, що сервер справжній. Діалог із групи 2 ведеться всередині цього захисту, тому група нижча. У запиті до `http://neverssl.com` цих рядків немає зовсім, а при помилках сертифіката діалог узагалі не почався. |
| 4 | З'ясування адреси сервера за назвою | **У виводах curl:**<br>`Host ... was resolved`<br>`IPv6: (none)`<br>`IPv4: ...`<br><br>**У виводі Resolve-DnsName:**<br>записи `A` і `AAAA`<br>`Section: Answer`<br>`TTL 300` (перше виконання)<br>`TTL 254` (повторне виконання) | Комп'ютер знає лише назву сайту, а підключитися можна лише за числовою адресою, тому спершу питають DNS. У виводі `Resolve-DnsName` видно саму відповідь DNS разом із часом, скільки її можна тримати в кеші (TTL). |
| 5 | Канал між двома комп'ютерами (адреси й порти) | **Спроба підключення:**<br>`Trying 172.66.175.59:443...`<br>та аналогічні рядки `Trying` в інших виводах<br><br>**З'єднання встановлено:**<br>`Established connection to example.net (172.66.175.59 port 443) from 192.168.3.173 port 57044`<br>`Established connection to neverssl.com (34.223.124.45 port 80) from 192.168.3.173 port 56594`<br>`Established connection to google.com (142.250.120.139 port 443) from 192.168.3.173 port 55248` | Видно двох учасників: ваш комп'ютер (адреса `192.168.3.173` і тимчасовий порт) та сервер (його адреса й порт 443 або 80). Це найнижче, що видно у виводах, і все вище працює поверх цього каналу. |

## Рядки, які не вдалося віднести до жодної групи:

| Рядок виводу | Причина утруднення |
|---|---|
| `* Request completely sent off` | Програма curl повідомляє, що запит повністю відправлено на сервер (буфер вихідних даних порожній). Це повідомлення про внутрішній стан програми, а не про мережевий пакет чи HTTP-заголовок, тому в жодну з груп воно не вписується. |
| `* Connection #0 to host google.com left intact` | Повідомлення від curl, що вона не закрила підключення одразу, а тримає його готовим на випадок наступного запиту. Це не подія обміну, а коментар програми про її власну поведінку. |
| `* closing connection #0` | Curl повідомляє, що закрила з'єднання після помилки. Дію виконала програма, але причиною є перевірка сертифіката, тому рядок опиняється між групою 3 (захист) і групою 5 (канал). |
| `curl: (35) schannel: ...`<br><br>`curl: (60) schannel: ...` | Підсумковий код помилки, який curl видає користувачу наприкінці. Сервер його не надсилав, це пояснення самої програми, чому вона зупинилася. |
| `* More details here: https://curl.se/docs/sslcerts.html` | Локальний текст, згенерований curl для користувача при помилці сертифіката. Із сервера він не приходив, тож не належить ні до захисту, ні до інших груп. |
