...
### Таблиця 1. Групи етапів (від найближчої до користувача до найближчої до апаратури)

| № | Назва групи (власне формулювання) | Рядки виводу, віднесені до групи | Обґрунтування |
|---|---|---|---|
| 1 | Команда, яку ввела людина | `C:\Users\Дар'я>curl -v https://example.net` (A.1), `curl -v http://neverssl.com` (A.2), `Resolve-DnsName example.net` (A.3), `curl -v https://google.com` (A.4), `curl -v https://expired.badssl.com`, `curl -v https://wrong.host.badssl.com`, `curl -v https://self-signed.badssl.com` (A.5) | Це єдині рядки, які написала людина. Усе інше у виводах є реакцією комп'ютерів на ці команди, тому група найближча до користувача. |
| 2 | Просимо сторінку і отримуємо відповідь  | Запит: `using HTTP/1.x`, `> GET / HTTP/1.1`, `> Host: ...`, `> User-Agent: curl/8.21.0`, `> Accept: */*`. Відповідь: `< HTTP/1.1 200 OK` (A.1, A.2), `< HTTP/1.1 301 Moved Permanently` і `< Location: https://www.google.com/` (A.4), заголовки `< Date`, `< Content-Type`, `< Server`, `< Cache-Control`, `< cf-cache-status: HIT`, `< Alt-Svc` та інші рядки з `<`. Вміст: `<!doctype html>...` (A.1), `<html>...NeverSSL...` (A.2), `<HTML>...301 Moved...` (A.4) | Клієнт просить сторінку (`>`), сервер називає результат (200 чи 301), описує відповідь у заголовках і віддає вміст (`<`). Результат цього діалогу людина бачить у браузері, тому група розташована одразу після команди. |
| 3 | Робимо обмін закритим від сторонніх і перевіряємо сервер | `schannel: disabled automatic use of client certificate`, `ALPN: curl offers http/1.1`, `ALPN: server accepted http/1.1`, `schannel: remote party requests renegotiation`, `schannel: renegotiating SSL/TLS connection`, `schannel: SSL/TLS connection renegotiated` (A.1, A.4). Збої перевірки в A.5: `SEC_E_CERT_EXPIRED`, `SEC_E_WRONG_PRINCIPAL`, `SEC_E_UNTRUSTED_ROOT` | Сторони узгоджують шифрування, а клієнт перевіряє, що сервер справжній. Діалог із групи 2 ведеться всередині цього захисту, тому група нижча. У A.2 (`http://`) цих рядків немає зовсім, а в A.5 через помилку перевірки діалог узагалі не почався. |
| 4 | Дізнаємося адресу за назвою | `Host ... was resolved`, `IPv6: (none)`, `IPv4: ...` (A.1, A.2, A.4, A.5). Таблиця `Resolve-DnsName` з A.3: записи `A` і `AAAA`, `Section: Answer`, `TTL 300` (перше виконання), `TTL 254` (повторне) | Комп'ютер знає лише назву сайту, а підключитися можна лише за числовою адресою, тому спершу питають DNS. У A.3 видно саму відповідь DNS разом із часом, скільки її можна тримати в кеші (TTL). |
| 5 | З'єднуємося й тримаємо з'єднання (адреси й порти) | `Trying 172.66.175.59:443...` (A.1) та аналогічні рядки в A.2, A.4, A.5. `Established connection to example.net (172.66.175.59 port 443) from 192.168.3.173 port 57044` (A.1), `... port 56594` (A.2), `... port 55248` (A.4) | Видно двох учасників: ваш комп'ютер (адреса `192.168.3.173` і тимчасовий порт) та сервер (його адреса й порт 443 або 80). Це найнижче, що видно у виводах, і все вище працює поверх цього каналу. |


### Таблиця 2. Рядки виводу, які не вдалося віднести до жодної групи
...
| `* closing connection #0` (A.5) | Curl повідомляє, що закрила з'єднання після помилки. Дію виконала програма, але причиною є перевірка сертифіката, тому рядок опиняється між групою 3 (захист) і групою 5 (канал). |
| `curl: (35) schannel: ...` і `curl: (60) schannel: ...` (A.5) | Підсумковий код помилки, який curl видає користувачу наприкінці. Сервер його не надсилав, це пояснення самої програми, чому вона зупинилася. |
| `* More details here: https://curl.se/docs/sslcerts.html` (A.5) | Локальний текст, згенерований curl для користувача при помилці сертифіката. Із сервера він не приходив, тож не належить ні до захисту, ні до інших груп. |
