...
| 2 | Діалог із веб-сервером: що просимо й що отримуємо | `* using HTTP/1.x`<br>`> GET / HTTP/1.1`<br>`> Host: example.net`<br>`> User-Agent: curl/8.21.0`<br>`> Accept: */*`<br><br>`< HTTP/1.1 200 OK`<br>`< Date: Mon, 21 Sep 2026 06:39:10 GMT`<br>`< Content-Type: text/html`<br>`< Server: cloudflare`<br>`< Cache-Control: max-age=14400`<br>`< cf-cache-status: HIT`<br><br>`< HTTP/1.1 301 Moved Permanently`<br>`< Location: https://www.google.com/`<br>`< Alt-Svc: h3=":443"; ma=2592000,h3-29=":443"; ma=2592000`<br><br>`<!doctype html><html lang="en"><head><title>Example Domain</title>...`<br><br>`<html><head><title>NeverSSL - Connecting ... </title>...`<br><br>`<HTML><HEAD><meta http-equiv="content-type" content="text/html;charset=utf-8">...` | Клієнт просить сторінку (рядки з `>`), а сервер відповідає (рядки з `<`): каже, чи все добре, і віддає саму сторінку. Результат цього діалогу людина бачить у браузері. Тому група стоїть одразу після команди. |
| 3 | Робимо обмін закритим від сторонніх і перевіряємо сервер | `* schannel: disabled automatic use of client certificate`<br>`* ALPN: curl offers http/1.1`<br>`* ALPN: server accepted http/1.1`<br>`* schannel: remote party requests renegotiation`<br>`* schannel: renegotiating SSL/TLS connection`<br>`* schannel: SSL/TLS connection renegotiated`<br><br>`* schannel: next InitializeSecurityContext failed: SEC_E_CERT_EXPIRED (0x80090328) - The received certificate has expired.`<br><br>`* schannel: SNI or certificate check failed: SEC_E_WRONG_PRINCIPAL (0x80090322) - The target principal name is incorrect.`<br><br>`* schannel: SEC_E_UNTRUSTED_ROOT (0x80090325) - The certificate chain was issued by an authority that is not trusted.` | Тут комп'ютер і сервер домовляються, як шифрувати дані, а комп'ютер перевіряє, що сервер справжній. Розмова з групи 2 йде вже всередині цього захисту, тому група нижча. |
| 4 | З'ясування адреси сервера за назвою | `* Host example.net:443 was resolved.`<br>`* IPv6: (none)`<br>`* IPv4: 172.66.175.59, 104.20.21.8`<br><br>`example.net AAAA 300 Answer 2606:4700:10::ac42:af3b`<br>`example.net A 300 Answer 172.66.175.59`<br><br>`example.net AAAA 254 Answer 2606:4700:10::ac42:af3b`<br>`example.net A 254 Answer 172.66.175.59` | Комп'ютер знає тільки назву сайту, а підключитися можна лише за числовою адресою. Тому спершу він питає DNS, і той повертає адресу. |
| 5 | З'єднуємося й тримаємо з'єднання (адреси й порти) | `*   Trying 172.66.175.59:443...`<br>`* Established connection to example.net (172.66.175.59 port 443) from 192.168.3.173 port 57044`<br><br>`*   Trying 34.223.124.45:80...`<br>`* Established connection to neverssl.com (34.223.124.45 port 80) from 192.168.3.173 port 56594`<br><br>`*   Trying 142.250.120.139:443...`<br>`* Established connection to google.com (142.250.120.139 port 443) from 192.168.3.173 port 55248` | Тут видно, хто з ким з'єднався: комп'ютер та сервер. Це найнижче, що видно у виводах. Усе, що вище, працює через це з'єднання. |

## Рядки, які не вдалося віднести до жодної групи:

| Рядок виводу | Причина утруднення |
|---|---|
| `* Request completely sent off` | Службове повідомлення curl про те, що запит повністю відправлено. Це не мережевий протокол і не HTTP-заголовок, а просто слова програми про її стан. |
| `* Connection #0 to host google.com left intact` | Службова інформація curl про те, що вона не закрила з'єднання після обміну, а залишила його на випадок наступного запиту. |
| `* closing connection #0` | Curl повідомляє, що закрила з'єднання після помилки. Дію зробила програма, а причина в перевірці сертифіката, тому рядок підходить і до групи 3, і до групи 5. |


## Контрольні питання
### 1. Скільки рядків діагностичного виводу передує отриманню даних сторінки (завдання A.1)?
> Отриманню даних сторінки передує 32 рядки діагностичного виводу — від визначення IP-адреси сервера до заголовків HTTP-відповіді включно.
### 2. Які рядки наявні у виводі A.1 і відсутні у виводі A.2? Чим це зумовлено?
> У `https://example.net` є рядки про TLS (`schannel`, `ALPN`, `renegotiating`), а в `http://neverssl.com` їх немає — бо `https` шифрує з'єднання, а `http` ні.
### 3. Звідки у виводі з'явилося значення 443, якщо його не було вказано в адресі?
> Порт 443 з'явився сам, бо це стандартний порт для `https://`. Якщо порт не вказати явно, програма підставляє його автоматично залежно від протоколу.
### 4. Як змінилося значення TTL між двома запитами (A.3)? Що означає це число?
> Значення TTL зменшилося з 300 до 254 (тобто приблизно на стільки секунд, скільки минуло між запитами). TTL (time to live) — це час у секундах, протягом якого DNS-запис можна зберігати в кеші, перш ніж запитувати його заново; зменшення числа показує, що резолвер віддав запис із кешу й порахував, скільки часу до нього ще лишилося.
...
| `wrong.host` | Сертифікат виданий на інше доменне ім'я, ніж те, до якого звертаємось. |
| `self-signed` | Сертифікат підписаний не довіреним центром сертифікації, а самим власником сайту. |

### 6. Три рядки з власних виводів, про які не йшлося на лекції 1
| № | Рядок виводу | Джерело (номер завдання) |
|---|---|---|
| 1 | `< cf-cache-status: HIT` | A.1 |
| 2 | `< Alt-Svc: h3=":443"; ma=2592000,h3-29=":443"; ma=2592000` | A.4 |
| 3 | `TTL 300` → `TTL 254` (значення TTL при повторному запиті) | A.3 |
